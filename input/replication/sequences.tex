...
\item [\textbf{Logoot~\cite{weiss2009logoot, weiss2010logootundo} :}] représente
  ...
  $\textsc{insert}(id_{précedent},\, element,\, id_{suivant})$ s'effectue en
  ...
  $2^{64}$ dans le modèle d'arbre). Ayant fait l'assomption que les documents
  étaient édités de gauche à droite, la stratégie s'avère inefficace dans le cas
  contraire. Le pire cas est déjà illustré dans la
...
\item [\textbf{Treedoc~\cite{shapiro2011comprehensive} :}] représente le
  ...
  succède 'W'. En d'autres termes, suffixer le chemin d'un 0 signifie \emph{à
    gauche de}, et suffixer le chemin d'un 1 signifie \emph{à droite
    de}. \TODO{Moar.}
\end{itemize}
