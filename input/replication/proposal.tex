...
% pour les structures de données sans résolution
%de conflits dédiées aux séquences.
Elle est composée de trois composants. Le premier composant est un structure
d'arbre dont l'arité maximale augmente avec sa profondeur. Le second composant
comprend deux sous-fonctions d'allocation conçues pour gérer des comportements
d'édition opposés. Le troisième composant est une fonction assignant à chaque
profondeur de l'arbre une sous-fonction d'allocation parmi celles disponibles.

Pris individuellement, ces composants n'arrivent pas à pourvoir des identifiants
de taille satisfaisante. Utilisés simultanément, les défaillances des uns sont
comblées par les forces des autres.  L'idée générale de \LSEQ consiste à
accepter la perte de niveaux des chemins composants ses identifiants pour peu
que les futures allocations compensent ces pertes.

Cette section commence par présenter les instructions composant l'algorithme de
\LSEQ. Ensuite, elle présente chacun des trois composants
individuellement. Enfin, elle résume le fonctionnement de \LSEQ au travers un
exemple.
...
exponentiel (cf. §\ref{repl:subsec:exponentialtree}) contenant assez d'espace
pour accueillir le nouvel élément.  Ensuite, la fonction de hachage $h$
