...
concurrentes. Ces marqueurs sont généralement constitués d'un identifiant de
site unique associé à une horloge de Lamport~\cite{lamport1978time}. Chaque
paire $\langle chemin,\, element \rangle$ est associée à un désambiguateur.
...
garantir des chemins dont la taille est sous-linéaire par rapport aux nombres
d'insertions dans la séquence. Logoot et Treedoc échouent à fournir des
identifiants bornés de cette façon dans les cas d'insertions de gauche à droite
et de droite à gauche. Ces deux approchent nécessitent l'exécution périodique
d'un mécanisme de relocalisation des identifiants afin de conserver de bonnes
performances.
