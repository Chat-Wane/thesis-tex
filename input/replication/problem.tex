...
\begin{problem}
  ...
  séquence. La fonction \textsc{allocPath} doit allouer des chemins tels que :
  \begin{equation}
    {|s(I+1)|\over{|s(I)|}} < \mathcal{O}(I)
  \end{equation}
\end{problem}

En d'autres termes, les chemins allouées par la fonction d'allocation
\textsc{allocPath} garantissent des identifiants dont la taille est
sous-linéaire par rapport aux nombres d'insertions dans la séquence. Logoot et
Treedoc échouent à fournir des identifiants bornés de cette façon dans les cas
d'insertions de gauche à droite et de droite à gauche. Ces deux approchent
nécessiten l'exécution périodique d'un mécanisme de relocalisation des
identifiants afin de conserver de bonnes performances.
...
d'insertions effectuées dans la séquence. À ce titre, aucun mécanisme
