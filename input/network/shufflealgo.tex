...
\begin{algorithmic}[1]
  ...
  \act
    ...
    \State \textbf{let} $sample \leftarrow $ \label{net:line:samplesize}
    $\textsc{getSample}(P \setminus\left\{\langle q, age\rangle\right\}, \left \lceil{|P|\over{2}} \right \rceil-1) \uplus \left\{\langle p, 0 \rangle\right\}$;
    \State $sample \leftarrow \textsc{replace}(sample,\,q,\,p)$;
  ...
  \pas
    ...
    \State \textbf{let} $sample' \leftarrow \textsc{getSample}(P ,\, \left\lceil |P|\over{2} \right\rceil )$;
